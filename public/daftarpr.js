// public/daftarpr.js (Final dengan Tombol Cetak Dinamis)
...
async function loadPurchaseRequests() {
    ...
    tableBody.innerHTML = '<tr><td colspan="5" class="loading-text">Memuat data...</td></tr>';
    ...
    try {
        ...
        const response = await fetch(`/api/requests?userName=${encodeURIComponent(userName)}&userRole=${userRole}`);
        ...
        if (requests.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="5" class="loading-text">Tidak ada data PR untuk Anda saat ini.</td></tr>';
        } else {
            requests.forEach(req => {
                let actionButtons = '';
                const canApprove = (req.status === 'Pending KTU Approval' && userRole === 'ktu') || (req.status === 'Pending Manager Approval' && userRole === 'manager');
                ...
                if (canApprove) {
                    ...
                    // --- LOGIKA UTAMA UNTUK TOMBOL CETAK ---
                    // Tentukan kelas CSS berdasarkan status
                    const buttonClass = req.status === 'Fully Approved' ? 'btn-sm btn-print' : 'btn-sm';
                    // Tentukan apakah tombol harus dinonaktifkan
                    const isDisabled = req.status !== 'Fully Approved' ? 'disabled' : '';

                    actionButtons = `<button class="${buttonClass}" onclick="printRequest(${req.id})" ${isDisabled}><i class="fa-solid fa-print"></i> Cetak</button>`;
                }
                ...
                const row = `
                    <tr>
                        ...
                        <td><span class="status ${req.status.toLowerCase().replace(/ /g, '-')}">${req.status}</span></td>
...
function filterTable() {
    ...
    const rows = document.getElementById('requests-table-body').getElementsByTagName('tr');
    for (let row of rows) {
        if (row.getElementsByTagName('td').length > 1) {
            ...
            const textValue = (prNoCell.textContent || prNoCell.innerText) + (uraianCell.textContent || uraianCell.innerText);
...
async function printRequest(id) {
    try {
        ...
        const checkElement = (elId) => {
            ...
            if (!el) throw new Error(`Elemen HTML dengan ID '${elId}' tidak ditemukan.`);
            return el;
        };

        checkElement('print-pr-no').textContent = request.prNo;
        checkElement('print-tanggal').textContent = new Date(request.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
        checkElement('print-keperluan').textContent = request.keperluan;
        checkElement('print-name-1').textContent = approvers['Diminta Oleh'] || ' ';
        checkElement('print-name-2').textContent = approvers['Diperiksa Oleh'] || ' ';
        checkElement('print-name-3').textContent = approvers['Disetujui Oleh'] || ' ';
        ...
        request.items.forEach((item, index) => {
            ...
            newRow.innerHTML = `<td>${index + 1}</td><td></td><td>${item.material}</td><td>${item.qty}</td><td>${item.satuan}</td><td>${(index === 0) ? request.keperluan : ''}</td>`;
        });
